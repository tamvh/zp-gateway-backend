    ...
    private void processs(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        switch (cmd) {            
            ...
            case "forget_password":
                content = forgetPassword(req, data);
                break;
            case "reset_password":
                content = resetPassword(req, data);
                break;
        }
...
//                String sessionId = req.getSession().getId();
                ...
                if (userName.isEmpty() || old_password.isEmpty() || new_password.isEmpty()) {
                    ...
                    if(ret == 0) {
                        ...
                        content = CommonModel.FormatResponse(AppConst.ERROR_CUR_PASSWORD_NOT_MATCHING, "Mật khẩu hiện tại không đúng", ret);
                    } else {
    ...
    private String forgetPassword(HttpServletRequest req, String data) {
        String content;
        int ret = AppConst.ERROR_GENERIC;
        try {
            JsonObject jsonObject = JsonParserUtil.parseJsonObject(data);
            if (jsonObject == null) {
                ...
                if (email.isEmpty()) {
                    ...
                    ret = AccountModel.getInstance().forgetPassword(email);
                    if(ret == 0) {
                        content = CommonModel.FormatResponse(AppConst.NO_ERROR, "", ret);
                    } else if (ret == 1) {
                        content = CommonModel.FormatResponse(AppConst.ERROR_INVALID_USER, "Tài khoản không tồn tại trên hệ thống hoặc đã bị khóa");
                    } else {
                        content = CommonModel.FormatResponse(AppConst.ERROR_GENERIC, "ERROR_GENERIC");
                    }                   
                }
            }
        } catch (Exception ex) {
    ...
    private String resetPassword(HttpServletRequest req, String data) {
        String content;
        int ret = AppConst.ERROR_GENERIC;
        try {
            JsonObject jsonObject = JsonParserUtil.parseJsonObject(data);
            if (jsonObject == null) {
                ...
                String userNameKey = jsonObject.get("u").getAsString();
                String password = jsonObject.get("p").getAsString();                
                String userName = CommonFunction.decode(userNameKey);

                if (userName.isEmpty() || password.isEmpty()) {
                    content = CommonModel.FormatResponse(ret, "Invalid parameter");
                } else {
                    ret = AccountModel.getInstance().resetPassword(userName, password);
                    if(ret == 0) {
                        content = CommonModel.FormatResponse(AppConst.NO_ERROR, "", ret);
                    } else {
                        content = CommonModel.FormatResponse(AppConst.ERROR_CHANGE_PASSWORD, "Đổi mật khẩu thất bại");
